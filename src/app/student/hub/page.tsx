...
function formatDate(d: string) {
  ...
  if (diffDays < 7)
    return `${date.toLocaleDateString("ar-SA", { weekday: "long" })} · ${timeStr}`;
  return (
        ...
        {busy ? (
          ...
          byDay.map((group) => (
            <div key={group.day}>
              ...
              {group.items.map((r) => {
                // In RTL: isMe → RIGHT side; in LTR: isMe → LEFT side
                // User asked: "my message left, others right" for LTR
                // For RTL Arabic: my message right, others left (natural)
                const isMe = r.author.id === me.id;
                const isStaff =
                  r.author.role === "TEACHER" ||
                  r.author.role === "SCHOOL_ADMIN";

                // Bubble side: isMe = right in RTL, left in LTR
                return (
                  <div
                    key={r.id}
...
// ─── POST CARD (group chat bubble style) ──────────────────────────────────────
  ...
  return (
    ...
    >
      ...
      <div className="chat-col">
        ...
        >
          {(canDelete || isMe) && (
            <button className="bubble-del" onClick={del} title="حذف">
              <svg
                width="12"
                height="12"
                viewBox="0 0 24 24"
      ...
      <div className="composer-row">
        ...
        <div className="composer-field-wrap">
          <textarea
            ...
            placeholder="شارك شيئاً مع المجموعة..."
            value={text}
            onChange={(e) => setText(e.target.value)}
      ...
      <header className="hub-header" ref={topRef}>
        <div className="hub-header-inner">
          <div className="hub-brand">
            ...
            <div>
              <h1 className="hub-title">المجتمع</h1>
      ...
      <div className="hub-feed" ref={feedRef}>
        ...
        {posts.length === 0 ? (
          <div className="hub-empty">
            ...
            <h3 className="hub-empty-title">كن أول من يكتب!</h3>
            <p className="hub-empty-sub">ابدأ المحادثة مع زملائك</p>
          </div>
        ) : (
          byDay.map((group) => (
            <div key={group.day}>
...
:root{
  ...
  --maroon2:#8F2424;
  --maroon3:#A32B2B;
  --maroon-dim:rgba(122,30,30,.08);
  --maroon-mid:rgba(122,30,30,.15);
  --maroon-border:rgba(122,30,30,.25);

  /* Gold */
  --gold:#C8A96A;
  --gold2:#D4B87A;
  --gold3:#E0C98A;
  --gold-dim:rgba(200,169,106,.1);
  --gold-border:rgba(200,169,106,.35);
  ...
  /* Their bubble (white-ish) */
  --their-bg:#FFFFFF;
  --their-fg:#2A1A0A;
  --their-border:rgba(0,0,0,.08);

  /* Staff bubble */
  --staff-bg:#FFF5F5;
  --staff-border:rgba(122,30,30,.18);
...
/* subtle warm pattern bg */
.hub-bg-pattern{
  position:absolute;inset:0;pointer-events:none;z-index:0;
  background-image:
    radial-gradient(circle at 20% 20%,rgba(200,169,106,.06) 0%,transparent 50%),
    radial-gradient(circle at 80% 80%,rgba(122,30,30,.04) 0%,transparent 50%);
}
...
.skel-av{width:42px;height:42px;border-radius:50%;background:var(--cream3);flex-shrink:0;animation:shimmer 1.5s ease infinite;background-size:600px 100%;}
.skel-bubble{
...
.hub-header{
  ...
  box-shadow:0 2px 20px rgba(122,30,30,.35);
}
.hub-header::after{
...
.hub-brand-icon{
  ...
  background:rgba(200,169,106,.2);border:1.5px solid rgba(200,169,106,.35);
  display:flex;align-items:center;justify-content:center;color:var(--gold);
  transition:all .3s var(--ease-b);
}
.hub-brand-icon:hover{background:rgba(200,169,106,.3);transform:scale(1.08) rotate(-4deg);}
.hub-title{font-size:17px;font-weight:700;color:#F7EDD8;letter-spacing:-.2px;line-height:1.25;}
.hub-subtitle{font-size:11px;color:rgba(200,169,106,.65);font-weight:500;margin-top:3px;}
.hub-live-badge{
...
.hub-live-dot{width:7px;height:7px;border-radius:50%;background:#6EE7B7;animation:pulse 2s ease-in-out infinite;flex-shrink:0;}
...
.chat-row{
  ...
  animation:blobIn .35s var(--ease-b) backwards;
}
...
/* Mine (gold, right in RTL, left in LTR) */
.chat-bubble-mine{
  background:var(--mine-bg);color:var(--mine-fg);
  border-bottom-right-radius:5px;
  box-shadow:0 3px 16px rgba(200,169,106,.3),0 1px 4px rgba(200,169,106,.2);
}
[dir="rtl"] .chat-bubble-mine{
  border-bottom-right-radius:18px;
  border-bottom-left-radius:5px;
}

/* Theirs (white card, left in RTL, right in LTR) */
.chat-bubble-theirs{
...
[dir="rtl"] .chat-bubble-theirs{
  border-bottom-left-radius:18px;
  border-bottom-right-radius:5px;
}

/* Staff bubble (slight red tint) */
.chat-bubble-staff{
...
/* Chat image */
.chat-img-wrap{
  margin-top:8px;border-radius:var(--r-sm);overflow:hidden;
  cursor:zoom-in;max-height:280px;transition:max-height .4s var(--ease);
}
...
.chat-time-row-mine{justify-content:flex-end;}
...
.chat-bubble:hover .bubble-del{opacity:.6;}
.bubble-del:hover{opacity:1 !important;background:rgba(200,0,0,.15);color:#c00;}

/* ── ACTIONS (reactions + reply button) ── */
...
/* Msg row (replies - also group chat style) */
...
.del-micro:hover{color:#d00;background:rgba(200,0,0,.08);}
...
.reply-input{
  ...
  background:#fff;outline:none;transition:all .2s var(--ease);
}
...
.reply-icon-btn:hover{border-color:var(--gold);color:var(--maroon);background:var(--gold-dim);}
.reply-send-btn{
  background:var(--maroon);border:none;color:#F7EDD8;
  width:36px;height:36px;border-radius:50%;cursor:pointer;flex-shrink:0;
  display:flex;align-items:center;justify-content:center;
  transition:all .2s var(--ease-b);
  box-shadow:0 3px 14px rgba(122,30,30,.3);
}
.reply-send-btn:hover:not(:disabled){transform:scale(1.1);box-shadow:0 4px 18px rgba(122,30,30,.4);}
.reply-send-btn:disabled{opacity:.35;cursor:not-allowed;}

/* ── CHIPS ── */
.chip-staff{
  font-size:9px;font-weight:700;color:var(--maroon);
  background:var(--maroon-dim);border:1px solid var(--maroon-border);
...
.av{
  ...
  display:flex;align-items:center;justify-content:center;
  position:relative;
  box-shadow:0 2px 8px rgba(0,0,0,.15),0 0 0 2px rgba(255,255,255,.8);
...
.rx-picker{
  ...
  animation:popIn .28s var(--ease-b) backwards;
}
...
.composer-send-btn{
  ...
  box-shadow:0 3px 16px rgba(122,30,30,.3);
}
.composer-send-btn:hover:not(:disabled){transform:scale(1.08);box-shadow:0 5px 20px rgba(122,30,30,.4);}
...
.load-more-btn:hover:not(:disabled){background:#fff;border-color:var(--gold);color:var(--maroon);}
...
@media(max-width:640px){
  ...
  .hub-brand-icon{width:40px;height:40px;}
